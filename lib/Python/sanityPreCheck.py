...
class RunPreCheck(SetMeUp):
    ...
    def test_existenz(self):
        msg = '{} already exists'.format(self.clbdirc)
        assert not self.clbdirc.exists(), msg
    ...
    def test_filePaths(self):
        assert os.path.exists(
            self.hydrorestart) or self.hydrorestart == 'None', "hydro restart file {} not found (or it isn't set to None)".format(self.hydrorestart)
        assert os.path.exists(
            self.hrldasrestart) or self.hrldasrestart == 'None', "hrldas restart file {} not found (or it isn't set to None)".format(self.hydrorestart)
        assert os.path.exists(
            self.parmdirc), "parameter directory {} does not exists".format(self.parmdirc)
        assert os.path.exists(
            self.exedirc), "parameter directory {} does not exists".format(self.exedirc)
    ...
    def run_all(self):
        ...
        testList = [method for method in dir(
            self.__class__) if method.startswith('test_')]
        ...
        logger.info("{} out of {} tests passed".format(
...
class RunPreSubmitTest(SetMeUp):
    ...
    def test_modeldirectory(self):
        ...
        assert os.path.exists(
            self.clbdirc), '{} does not exist'.format(self.clbdirc)

        # ensure that the correct files exist in the clbdirc directory
        withext_filelist_clbdirc = [
            "{}/{}".format(self.clbdirc, x) for x in self.filelist_clbdirc]
        ...
        withext_filelist_clbdirc = [
            "{}/DOMAIN/{}".format(self.clbdirc, x) for x in self.filelist_domain]
        for fl in withext_filelist_clbdirc:
            assert os.path.exists(fl), "{} not found".format(fl)
    ...
    def test_forcing_we_dim(self):
        ...
        # forcigns
        # CHANGE ME -- THIS IS SLOW
        forcingFilename = glob.glob('{}/FORCING/*'.format(self.clbdirc))[0]
    ...
    def test_observations(self):
        ...
        assert os.path.isfile(
            observation_file), "{} not found".format(observation_file)
